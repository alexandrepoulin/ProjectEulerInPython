...
## find the recusion:
import dioph
rec = dioph.solveRecurrence(5,0,-1,14,0,1)

## the independent solutions are:
solutions = dioph.solve(5,0,-1,14,0,1)
##some issues, just fixing them
solutions = [x for x in filter(lambda k: (5*k[0]**2-k[1]**2 +14*k[0]+1) == 0 ,solutions)]
...
for s in solutions:
    current = s
    for i in range(30): ##30 should be enough
        current = nextSolTemp(current)
        nuggets.append(current)

##need to sort the nuggets based on first index
sortedNuggets = [x for x in filter(lambda k: (5*k[0]**2-k[1]**2 -14*k[0]+1) == 0 ,nuggets)]
